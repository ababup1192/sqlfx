...
3. `withTx` 完成: ネスト（セーブポイント or 型エラー、ここで決める）・読み取り専用・分離レベル
   - [x] `Tx.withSavepoint`（Tx の一部分を SAVEPOINT で囲み、中の失敗を値にして `ROLLBACK TO` で続ける。`SqlSavepoint` effect。読むだけの Tx でも張れる）
...
| 未決 | 決めるフェーズ | 決め方 |
|---|---|---|
| Tx ネスト（セーブポイント vs 型エラー） | 3 | フェーズ 1 では型エラー固定。部分的な巻き戻しは `Tx.withSavepoint` で先に出した |
