...
Everything lives under `Sqlfx`: the modules are `Sqlfx.Sql`, `Sqlfx.Row`, `Sqlfx.Decoder`,
`Sqlfx.Pool` and so on, and the effects are `Sqlfx.SqlRead`, `Sqlfx.DbErr` and their neighbours.
You bring in the ones a file uses and write the short name after that:
...
Every snippet below is written that way, so `Sql.fetch` and `\ DbRead` read as they do in your own
file. Names are spelled out in full only where it matters which module they come from.
...
```flix
use Sqlfx.Fragment.{===, >>};   // once, at the top of the file or the module
...
`Sqlfx.Timestamp` / `Sqlfx.Date` / `Sqlfx.Zone` / `Sqlfx.Format` / `Sqlfx.Uuid` do not depend on the
database layer.
...
carries only the library (`src/Sqlfx.flix` and `src/Sqlfx/`, without `src/Sqlfx/Q/`) — the generator
has a `main` behind it and nothing in a consumer calls it from Flix.
...
- The generated files carry a generator version in their header (`// GENERATED by sqlfx gen v10 …`).
...
are unchanged, so the edit is the `use` lines: add `use Sqlfx.Sql`, `use Sqlfx.DbRead` and the rest
at the top of each module, and `Sql.fetch` / `\ DbRead` in the body stay as they are. Run
`make gen` (or `bin/flix run -- gen ...`) to regenerate, because the generated files open with
`use Sqlfx....` now and the header is `v10`.

| 0.4.7 | 0.5.0 |
|---|---|
| Modules `Sql` `Row` `Decoder` `SqlValue` `Tx` `Pool` `Jdbc` `JdbcConvert` `SqlState` `Retry` `Preload` `Savepoint` `Migrate` `DbTest` `DbError` `DbErr` `TransientDbErr` `Db` `RawSql` `Fragment` `Timestamp` `Date` `Zone` `TimeZone` `TimeTest` `Format` `Unsafe` `Uuid` at the top level | the same names under `Sqlfx.` |
| Effects `SqlRead` `SqlWrite` `SqlSavepoint` `RawSql` `DbErr` `TransientDbErr` `TimeZone`, aliases `Db` `DbRead` `DbWrite` at the top level | `Sqlfx.SqlRead` and so on. With `use Sqlfx.DbRead` the signature stays `\ DbRead` |
| `Migrate.MigrateErr` | `Sqlfx.Migrate.MigrateErr` — it was already a nested effect, so only the module path changed |
| Generated files opened with `use SqlValue.SqlValue` / `use Decoder.Decoder` | They open with a block of `use Sqlfx....`. The module they declare (`UsersQueries`, `UsersTable`) is yours and did not move |
| The generator's own modules (`Codegen`, `Schema`, `QParser`, ...) sat at the top level, which is why they were kept out of the `.fpkg` | They are `Sqlfx.Q.*`. They are still not in the `.fpkg`, but a repository that vendors the sources no longer loses the name `Schema` |
...
```
src/Sqlfx.flix    the effects and their aliases: SqlRead / SqlWrite / DbErr / TransientDbErr / ...
src/Sqlfx/        layer 0 and the fragment DSL: SqlValue / Row / Decoder / Sql / DbError / Retry /
                  Tx / Preload / Fragment, the JDBC handlers (Jdbc / Pool / SqlState), the test
                  handlers (DbTest), and Timestamp / Date / Zone / Format / Uuid
src/Sqlfx/Q/      layer 1: the .q parser, schema resolution, the generator. Not in the .fpkg
...
全部が `Sqlfx` の下に在る。モジュールは `Sqlfx.Sql` / `Sqlfx.Row` / `Sqlfx.Decoder` / `Sqlfx.Pool`、
エフェクトは `Sqlfx.SqlRead` / `Sqlfx.DbErr` のように綴る。ファイルで使う物を `use` に並べ、
以降は短い名前で書く:
...
以下の例は全部この書き方なので、`Sql.fetch` や `\ DbRead` は手元のファイルと同じに読める。
どのモジュールの物かが問題になる所だけ、全部の名前で綴る。
...
```flix
use Sqlfx.Fragment.{===, >>};   // ファイルの先頭か mod の先頭に 1 回
...
`Sqlfx.Timestamp` / `Sqlfx.Date` / `Sqlfx.Zone` / `Sqlfx.Format` / `Sqlfx.Uuid` は DB の層に依存しない。
...
（`src/Sqlfx.flix` と `src/Sqlfx/`。`src/Sqlfx/Q/` は除く）だけ。生成器は `main` から動かす物で、
利用側の Flix のコードからは呼ばない。
...
全モジュールが `Sqlfx` の下に移った。関数も型も生成コードの形も変わらないので、直すのは `use` の
行だけ。各モジュールの先頭に `use Sqlfx.Sql` / `use Sqlfx.DbRead` のように並べれば、本文の
`Sql.fetch` や `\ DbRead` はそのままで通る。生成物は `use Sqlfx....` で始まる形になり
ヘッダが `v10` になるので、`make gen`（または `bin/flix run -- gen ...`）で作り直す。

| 0.4.7 | 0.5.0 |
|---|---|
| モジュール `Sql` `Row` `Decoder` `SqlValue` `Tx` `Pool` `Jdbc` `JdbcConvert` `SqlState` `Retry` `Preload` `Savepoint` `Migrate` `DbTest` `DbError` `DbErr` `TransientDbErr` `Db` `RawSql` `Fragment` `Timestamp` `Date` `Zone` `TimeZone` `TimeTest` `Format` `Unsafe` `Uuid` がトップレベル | 同じ名前が `Sqlfx.` の下 |
| エフェクト `SqlRead` `SqlWrite` `SqlSavepoint` `RawSql` `DbErr` `TransientDbErr` `TimeZone` と alias `Db` `DbRead` `DbWrite` がトップレベル | `Sqlfx.SqlRead` など。`use Sqlfx.DbRead` を書けば署名は `\ DbRead` のまま |
| `Migrate.MigrateErr` | `Sqlfx.Migrate.MigrateErr`。元から `Migrate` の中のエフェクトで、モジュールの道が伸びただけ |
| 生成物の先頭が `use SqlValue.SqlValue` / `use Decoder.Decoder` | `use Sqlfx....` の塊になる。生成物が宣言するモジュール（`UsersQueries` / `UsersTable`）は利用側の物なので動かない |
| 生成器のモジュール（`Codegen` / `Schema` / `QParser` ...）がトップレベルに在り、それが `.fpkg` から外していた理由 | `Sqlfx.Q.*` になった。`.fpkg` に入らないのは変わらないが、ソースを写して使うリポジトリでも `Schema` の名前を取られない |
...
```
src/Sqlfx.flix    エフェクトと alias: SqlRead / SqlWrite / DbErr / TransientDbErr / ...
src/Sqlfx/        層 0 と断片 DSL: SqlValue / Row / Decoder / Sql / DbError / Retry / Tx /
                  Preload / Fragment、JDBC のハンドラ（Jdbc / Pool / SqlState）、
                  テストのハンドラ（DbTest）、Timestamp / Date / Zone / Format / Uuid
src/Sqlfx/Q/      層 1: .q のパーサ / スキーマの解決 / 生成器。.fpkg には入らない
